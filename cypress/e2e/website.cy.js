    ...
    componentSelectors.forEach(selector => {
      ...
      section.then(element => {
        if (!element.is(':visible')) {
          section.scrollIntoView({ duration: 25 }).should('not.have.css', 'opacity', '0');
        }
      });
      ...
      section.find('*').each((element, index) => {
        if (!element.is(':visible')) {
          if (element.is('.hover-overlay')) {
            cy.get(element).scrollIntoView({ duration: 25 }).realHover().should('not.have.css', 'opacity', '0');
          }
          else {
            cy.get(element).scrollIntoView({ duration: 25 }).should('not.have.css', 'opacity', '0');
          }
        }
      });
    });
